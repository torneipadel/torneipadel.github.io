...
(function () {
  ...
  function costPanel(db, git) {
    ...
    const dbCost = dbOver * 0.125;
    const storageCost = storageOver * 0.021;
    const known = dbCost + storageCost;
    return '<div class="asm-section"><h3>💰 Costi e rischio di eccedenza</h3>' +
      '<div class="asm-cost-grid">' +
        ...
        '<div class="asm-cost-card"><b>Supabase — eventuale eccedenza</b><strong>' + (known > 0 ? '
    const s = status(p);
  ...
  window.apriMonitorSistema = open;
})(); + known.toFixed(2) + '/mese' : '$0 misurabile') + '</strong><span>Database oltre quota: $0,125/GB · Storage oltre quota: $0,021/GB. Egress e MAU richiedono Usage/Billing per il calcolo reale.</span></div>' +
        ...
        '<div class="asm-cost-card"><b>GitHub</b><strong>$0 per il repository pubblico</strong><span>La dimensione del repository non viene trattata come costo. Actions/Packages/LFS sono servizi con quote separate; il monitor li segnala come “non leggibili” finché GitHub non espone il billing tramite un collegamento autenticato.</span></div>' +
      '</div>' +
      '<div class="asm-note">⚠️ Il valore Supabase qui mostrato è una stima delle sole eccedenze calcolabili dal monitor. Non rappresenta una fattura reale. Per Egress, MAU, Actions, Packages e LFS non vengono inventati consumi o costi.</div>' +
    '</div>';
  }
