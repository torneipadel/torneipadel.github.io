...
async function load(){let list=[];const c=client();if(!c?.from){console.error('Sponsor pubblici: client Supabase non disponibile.');window.__PUBLIC_SPONSORS__=[];render([]);return}try{const r=await c.from('sponsor').select('id,nome,immagine,video,link').order('id',{ascending:true});if(r.error){console.error('Errore caricamento sponsor pubblici',r.error);window.__PUBLIC_SPONSORS__=[];render([]);return}list=(r.data||[]).filter(s=>s&&s.nome)}catch(e){console.error('Errore caricamento sponsor pubblici',e);window.__PUBLIC_SPONSORS__=[];render([]);return}window.__PUBLIC_SPONSORS__=list;render(list)}
