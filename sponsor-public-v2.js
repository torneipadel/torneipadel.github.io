...
function render(list){const tr=document.getElementById('sponsor-public-track');if(!tr)return;stop();if(!list.length){tr.innerHTML='<div class="sponsor-public-empty">Nessuno sponsor disponibile</div>';return}tr.innerHTML=list.map(s=>{const logo=s.immagine?`<span class="sponsor-public-logo-box"><img class="sponsor-public-logo" src="${esc(s.immagine)}" alt="${esc(s.nome)}" loading="lazy"></span>`:'';const name=`<span class="sponsor-public-name">${esc(s.nome)}</span>`;const html=logo+name;return s.link?`<a class="sponsor-public-item" href="${esc(s.link)}" target="_blank" rel="noopener noreferrer">${html}</a>`:`<div class="sponsor-public-item">${html}</div>`}).join('');bind();requestAnimationFrame(start)}
