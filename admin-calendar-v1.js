...
const allTournaments=()=>Array.isArray(state().tornei)?state().tornei.filter(t=>String(t?.stato||'').trim().toLowerCase()!=='archiviato'):[ ];
...
function tournamentCard(item){const t=item?.t||item,g=item?.g||null,d=parseDate(g?.data||tournamentDate(t)),selected=String(t.id)===String(state().torneoSelezionato),matches=Array.isArray(g?.partite)?g.partite:[],complete=matches.filter(m=>m?.risA!==''&&m?.risA!=null&&m?.risB!==''&&m?.risB!=null).length;return `<div class="agenda-card${selected?' current':''}"><div class="agenda-card-date"><strong>${d?d.toLocaleDateString('it-IT',{day:'2-digit',month:'short'}):'-'}</strong></div><div style="flex:1;min-width:0"><strong>${esc(t.nome||'Torneo')}${g?` · Giornata ${esc(g.numero)}`:''}</strong><div class="agenda-meta">${esc(t.ora_inizio||g?.ora||'')} ${t.posti?`· ${esc(t.posti)} squadre`:''}${g?` · ${matches.length} partite · ${complete}/${matches.length} risultati`:''} · ID ${esc(t.id)}</div></div><button type="button" class="btn" data-calendar-open="${esc(t.id)}">Apri torneo</button></div>`}
