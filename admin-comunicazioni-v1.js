...
$('newsImage')?.addEventListener('change',async()=>{const file=$('newsImage').files?.[0];if(!file)return;if(!['image/png','image/jpeg','image/webp','image/gif'].includes(file.type)){alert('L’immagine deve essere PNG, JPG, WEBP o GIF.');$('newsImage').value='';return}if(file.size>5*1024*1024){alert('L’immagine è troppo grande. Usa un file massimo di 5 MB.');$('newsImage').value='';return}try{const u=await fileToDataUrl2(file);$('newsPreview').innerHTML=`<div style="width:100%;max-width:320px;height:150px;border-radius:10px;overflow:hidden"><img src="${esc(u)}" style="width:100%;height:100%;object-fit:cover" alt="Anteprima"></div>`}catch(e){alert(e.message)}});
