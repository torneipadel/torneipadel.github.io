...
async function loginAdmin(){try{const{data,error}=await sb.auth.getSession();if(error)throw error;let session=data?.session;if(!session){const email=document.getElementById("adminEmail")?.value?.trim(),password=document.getElementById("adminPassword")?.value||"";if(!email||!password){mostraLoginMessaggio("Inserisci email e password.","#b42332");return}const result=await sb.auth.signInWithPassword({email,password});if(result.error)throw result.error;session=(await sb.auth.getSession()).data.session}if(!session?.user?.id){mostraLoginMessaggio("Accesso non riuscito.","#b42332");return}const{data:profilo,error:profiloError}=await sb.from("profili").select("ruolo").eq("user_id",session.user.id).maybeSingle();if(profiloError)throw profiloError;if(String(profilo?.ruolo||"").toLowerCase()!=="admin"){try{await sb.auth.signOut()}catch(_e){}adminState.adminLoggato=false;adminState.adminEmail="";window.adminState=adminState;salvaAdminState();document.getElementById("boxLoginAdmin")?.classList.remove("hidden");document.getElementById("areaAdmin")?.classList.add("hidden");mostraLoginMessaggio("Questo account non è autorizzato come amministratore.","#b42332");return}adminState.adminLoggato=true;adminState.adminEmail=session.user.email||"Admin";window.adminState=adminState;salvaAdminState();document.getElementById("boxLoginAdmin")?.classList.add("hidden");document.getElementById("areaAdmin")?.classList.remove("hidden");await caricaTorneiSupabase();await caricaRichiesteIscrizione();if(typeof window.renderCleanAdmin==="function")window.renderCleanAdmin()}catch(e){console.error("Errore login:",e);mostraLoginMessaggio(e?.message||"Accesso non riuscito.","#b42332")}}
...
function limiteIscrizioniTorneo(t){const formula=String(t?.formula||t?.configurazione?.rules?.formulaScelta||t?.configurazione?.rules?.tipoTorneo||"");if(formula==="individualeCoppieVariabili")return Math.max(1,Number(t?.posti)||Number(t?.configurazione?.rotazione?.numeroGiocatori)||8);return Math.max(1,(Number(t?.posti)||Number(t?.configurazione?.rules?.numeroSquadre)||Number(t?.configurazione?.numeroSquadre)||8)*2)}
function contaIscrizioniAttive(rows){return (Array.isArray(rows)?rows:[]).filter(x=>String(x?.stato||"").toLowerCase()!=="rifiutato").length}
async function caricaRichiesteIscrizione(){const id=Number(adminState.torneoSelezionato);if(!Number.isFinite(id)||id<=0){window.iscrizioniTorneo=[];return}try{const{data,error}=await sb.from("iscrizioni").select("*").eq("torneo_id",id).order("data_iscrizione",{ascending:false});if(error)throw error;window.iscrizioniTorneo=Array.isArray(data)?data:[]}catch(e){window.iscrizioniTorneo=[];console.error("Errore caricamento iscrizioni:",e)}}
