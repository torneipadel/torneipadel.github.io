...
function render(t,ps){
  ...
  const dayCard=(g,open=false)=>'<details class="card" style="margin:10px 0" '+(open?'open':'')+'><summary style="cursor:pointer;padding:16px;font-weight:700;display:flex;justify-content:space-between;gap:12px;list-style:none"><span>▼ Giornata '+g.numero+' — '+esc(g.data||'Data non impostata')+'</span><span class="notice">'+esc((g.partite||[]).length+' partite')+'</span></summary><div class="card-body"><div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap;margin-bottom:12px"><label>Data <input data-date="'+g.numero+'" type="date" value="'+esc(g.data||'')+'"></label><span class="notice">'+esc(g.riposo?.length?'Riposo: '+g.riposo.map(id=>pm[id]||'Giocatore').join(', '):'Nessun riposo')+'</span></div>'+(g.partite||[]).map((m,i)=>'<div class="list-item"><div style="flex:1"><strong>'+esc((m.coppiaA||[]).map(id=>pm[id]||'Giocatore').join(' / '))+' <span>VS</span> '+esc((m.coppiaB||[]).map(id=>pm[id]||'Giocatore').join(' / '))+'</strong><small style="display:block;margin-top:6px">Campo <input data-meta="'+g.numero+'" data-mi="'+i+'" data-k="campo" value="'+esc(m.campo||'')+'" style="width:90px"> · Ora <input data-meta="'+g.numero+'" data-mi="'+i+'" data-k="ora" type="time" value="'+esc(m.ora||'')+'" style="width:105px"></small><details style="margin-top:8px"><summary>✋ Gestione manuale coppie</summary><div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:8px">'+[0,1,2,3].map(pos=>'<select data-pair="'+g.numero+'" data-mi="'+i+'" data-pos="'+pos+'">'+ps.map(p=>'<option value="'+esc(key(p))+'" '+(key(p)===(pos<2?(m.coppiaA||[])[pos]:(m.coppiaB||[])[pos-2])?'selected':'')+'>'+esc(name(p))+'</option>').join('')+'</select>').join('')+'<button class="btn" data-save-pair="'+g.numero+'" data-mi="'+i+'">Salva coppie</button></div></details></div><div class="list-actions"><input data-r="'+g.numero+'" data-m="'+i+'" data-s="a" type="number" min="0" value="'+esc(m.risA)+'" placeholder="0"><span>—</span><input data-r="'+g.numero+'" data-m="'+i+'" data-s="b" type="number" min="0" value="'+esc(m.risB)+'" placeholder="0"></div></div>').join('')+'</div></details>';
  ...
  '<details class="card"><summary style="cursor:pointer;padding:16px;font-weight:700">▼ Configurazione</summary><div class="card-body"><div class="section-grid">'+
  '<div><label>Numero giocatori</label><input id="rotN" type="number" min="4" value="'+r.numeroGiocatori+'"></div><div><label>Punti vittoria</label><input id="rotPV" type="number" min="0" value="'+r.puntiVittoria+'"></div><div><label>Punti pareggio</label><input id="rotPP" type="number" min="0" value="'+r.puntiPareggio+'"></div><div><label>Punti sconfitta</label><input id="rotPS" type="number" min="0" value="'+r.puntiSconfitta+'"></div><div><label>Campo predefinito</label><input id="rotCampo" value="'+esc(r.campoDefault)+'"></div><div><label>Ora predefinita</label><input id="rotOra" type="time" value="'+esc(r.oraDefault)+'"></div><div><label>Numero giornate torneo</label><select id="rotNG"><option value="6" '+(r.numeroGiornate===6?'selected':'')+'>6</option><option value="8" '+(r.numeroGiornate===8?'selected':'')+'>8</option><option value="10" '+(r.numeroGiornate===10?'selected':'')+'>10</option><option value="12" '+(r.numeroGiornate===12?'selected':'')+'>12</option><option value="manuale" '+(r.numeroGiornate==='manuale'?'selected':'')+'>Manuale</option></select></div></div></div></details>'+
  '<details class="card"><summary style="cursor:pointer;padding:16px;font-weight:700">▼ Giocatori</summary><div class="card-body"><div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px"><button class="btn" id="rotSimulate">🧪 Crea simulazione completa</button><button class="btn primary" id="rotNewRound">＋ Genera nuova giornata</button></div><div class="notice">'+ps.length+' iscritti approvati</div><div class="list" style="margin-top:10px">'+ps.map(p=>{const s=rank.find(x=>x.id===key(p));return '<div class="list-item"><div><strong>'+esc(name(p))+'</strong><small>'+((s?.partite)||0)+' partite · '+((s?.punti)||0)+' punti · differenza '+((s?.differenza)||0)+'</small></div></div>'}).join('')+'</div></div></details>'+
  '<details class="card"><summary style="cursor:pointer;padding:16px;font-weight:700">▼ Calendario</summary><div class="card-body"><div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:10px">'+calendarDayInputs(r)+'</div><div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap"><label style="display:flex;align-items:center;gap:5px"><input id="rotCalActive" type="checkbox" '+(r.calendario.attivo?'checked':'')+'> Attiva calendario fisso</label><label>Ora ricorrente <input id="rotCalOra" type="time" value="'+esc(r.calendario.ora)+'"></label></div><small>Puoi programmare più giorni alla settimana con orari diversi. Data, ora e campo della singola giornata restano modificabili.</small><div class="notice" style="margin-top:10px">Programmazione: <b>'+esc(calendarText(r.calendario))+'</b></div><button class="btn primary" id="rotSaveRules" style="margin-top:12px">💾 Salva impostazioni</button><div class="notice" style="margin-top:10px">Numero giornate: <b id="rotLimitText"></b></div></div></details>'+
  '<details class="card" open><summary style="cursor:pointer;padding:16px;font-weight:700">📊 Classifica</summary><div class="card-body"><div class="notice" style="margin-bottom:10px">Punti → differenza → punti fatti → vittorie → partite giocate</div><div style="overflow:auto"><table style="width:100%"><thead><tr><th>#</th><th>Giocatore</th><th>Pt</th><th>PG</th><th>V</th><th>P</th><th>S</th><th>PF</th><th>PS</th><th>Diff.</th></tr></thead><tbody>'+rank.map((x,i)=>'<tr><td>'+(i+1)+'</td><td><b>'+esc(x.nome)+'</b></td><td>'+x.punti+'</td><td>'+x.partite+'</td><td>'+x.vittorie+'</td><td>'+x.pareggi+'</td><td>'+x.sconfitte+'</td><td>'+x.puntiFatti+'</td><td>'+x.puntiSubiti+'</td><td><b>'+x.differenza+'</b></td></tr>').join('')+'</tbody></table></div></div></details>'+
  '<details class="card"><summary style="cursor:pointer;padding:16px;font-weight:700">▼ Archivio risultati <span class="notice">('+archived.length+' giornate)</span></summary><div class="card-body">'+(archived.length?archived.map(g=>dayCard(g,false)).join(''):'<div class="empty">Nessuna giornata archiviata.</div>')+'</div></details>'+
  '<details class="card" '+(currentDay?'open':'')+'><summary style="cursor:pointer;padding:16px;font-weight:700">▼ Giornata in corso'+(currentDay?' — Giornata '+currentDay.numero:'')+'</summary><div class="card-body">'+(currentDay?dayCard(currentDay,true).replace(/^<details[^>]*>|<\/details>$/g,''):'<div class="empty">Nessuna giornata in corso. Genera una nuova giornata quando necessario.</div>')+'</div></details>'+
  '<details class="card"><summary style="cursor:pointer;padding:16px;font-weight:700">▼ Controllo rotazioni</summary><div class="card-body"><div class="info-row"><span>Partite per giocatore</span><b>'+minPlayed+' – '+maxPlayed+'</b></div><div class="info-row"><span>Coppie già registrate</span><b>'+pairCount+'</b></div><div class="info-row"><span>Giornate completate</span><b>'+r.giornate.filter(complete).length+' / '+r.giornate.length+'</b></div><div class="notice" style="margin-top:10px">La generazione automatica usa esclusivamente lo storico come base per la nuova giornata e non modifica le giornate concluse.</div></div></details>';
