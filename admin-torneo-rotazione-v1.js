...
function render(t,ps){
  ...
  $('rotNewRound').onclick=async()=>{try{const fresh=await players(t);if(Number(r.numeroGiocatori)>0&&fresh.length<Number(r.numeroGiocatori))throw Error('Servono '+r.numeroGiocatori+' giocatori approvati; al momento sono '+fresh.length+'.');const nc=generateRound(t,fresh);await save(t,nc);render(t,fresh)}catch(e){alert(e.message||e)}};
