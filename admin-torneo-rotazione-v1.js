...
function calendarDayInputs(r){return [1,2,3,4,5,6,7].map(n=>'<label style="display:flex;align-items:center;gap:5px"><input type="checkbox" class="rotDay" value="'+n+'" '+(r.calendario.giorni.includes(n)?'checked':'')+'> '+dayName(n)+'</label>').join('')}
...
function nextCalendarSlot(t){const c=config(t),cal=c.rotazione.calendario||{},giorni=Array.isArray(cal.giorni)?cal.giorni:[],ora=String(cal.ora||c.rotazione.oraDefault||'19:00');const last=(c.rotazione.giornate||[]).map(g=>parseDate(g.data)).filter(Boolean).sort((a,b)=>b-a)[0];const base=last?new Date(last.getTime()+86400000):new Date();if(!giorni.length)return {data:isoDate(base),ora:c.rotazione.oraDefault};for(let i=0;i<370;i++){const d=new Date(base.getTime()+i*86400000),js=d.getUTCDay(),giorno=js===0?7:js;if(giorni.includes(giorno))return {data:isoDate(d),ora};}return {data:isoDate(base),ora}}
function calendarText(cal){const giorni=Array.isArray(cal?.giorni)?cal.giorni:[];return giorni.length?giorni.map(dayName).join(' · ')+' alle '+String(cal.ora||''):'Non programmato'}
