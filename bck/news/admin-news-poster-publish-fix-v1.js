...
document.addEventListener('click',e=>{const b=e.target?.closest?.('#naiManualPublish');if(!b)return;e.preventDefault();e.stopImmediatePropagation();publishPoster()},true);
function bindDraft(){const b=q('#naiSaveDraft');if(!b||b.dataset.localDraftFix==='1')return;b.dataset.localDraftFix='1';b.onclick=()=>{const t=selected();const key='news-ai-draft-'+String(t?.id||'');const ids=['naiType','naiTitle','naiDate','naiTime','naiLocation','naiPairs','naiLevel','naiFee','naiDeadline','naiOffer','naiProduct','naiPrice','naiCta'];const draft={};ids.forEach(id=>{const e=q('#'+id);if(e)draft[id]=e.value});try{localStorage.setItem(key,JSON.stringify(draft));const status=q('#naiStatus');if(status)status.textContent='Bozza salvata localmente. Non è ancora pubblicata.'}catch(e){console.error('Salvataggio bozza locale:',e);alert('Impossibile salvare la bozza localmente.')}}}
const obs=new MutationObserver(bindDraft);obs.observe(document.body,{childList:true,subtree:true});setTimeout(bindDraft,500);window.addEventListener('admin:render',bindDraft);bindDraft();
function bindEditPublish(){const b=q('#naiPublish');if(!b)return;if((b.textContent||'').trim()==='💾 Aggiorna News'){b.textContent='🚀 Pubblica aggiornamento';b.title='Pubblica le modifiche della News esistente'}}
const editObs=new MutationObserver(bindEditPublish);editObs.observe(document.body,{childList:true,subtree:true,characterData:true});setTimeout(bindEditPublish,300);setInterval(bindEditPublish,1000);window.addEventListener('admin:render',bindEditPublish);bindEditPublish();window.__NAI_POSTER_PUBLISH_FIX__=true;
})();